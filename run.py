...
AUTH_KEYWORDS = ['ragging','purchase','bid','document','murder','negligence','rape','theft','misappropriation','kidnaping','murder','suicide','cheating','forgery']
sec=[185,464,302,'304A',376,379,403,363,307,309,465]
AUTH_NOT_REQD = -1
AUTH_NOT_SUCC = 0
AUTH_SUCC = 1
UNAME_REQ = 0
...
conn = sqlite3.connect('C:\\Users\\Goushik Ram\\Desktop\\chat bot\\seminar2_progress\shrya\\sqlite\db\\pythonsqlite.db')
...
def conv_mapping(inp):
	...
	return new_inp
def pll(idn):
	return("No i cant idesigned for college realted queries")
def providePersonalInfo(inp, idn):
	...
	if(('RAGGING') in p_inp):
		...
		conn.close() 
		return ragging(idn)
	elif(('PURCHASE' in p_inp) or ('BID' in p_inp)):
		c.close()
		conn.close() 
		return illegal(idn)
	elif(('DOCUMENT' in p_inp)):
		c.close()
		conn.close() 
		return document(idn)
	elif(('MURDER' in p_inp)):
		c.close()
		conn.close() 
		return murder(idn)
	elif(('NEGLIGENCE' in p_inp)):
		c.close()
		conn.close() 
		return death(idn)
	elif(('RAPE''HARASMENT' in p_inp)):
		c.close()
		conn.close() 
		return rape(idn)
	elif(('THEFT' in p_inp)):
		...
		conn.close() 
		return theft(idn)
	elif(('MISAPPROPRIATION' in p_inp)):
		c.close()
		conn.close() 
		return miss(idn)
	elif(('KIDANPING' in p_inp)):
		c.close()
		conn.close() 
		return kidnap(idn)
	elif(('SUICIDE' in p_inp)):
		c.close()
		conn.close() 
		return suicide(idn)
	elif(('CHEATING' in p_inp)):
		c.close()
		conn.close() 
		return cheating(idn)
	elif(('FORGERY' in p_inp)):
		c.close()
		conn.close() 
		return forgery(idn)
...
def matchingSentence(inp):
	...
	c = conn.cursor()
	f = open('E:\kabilan R\saveetha\chat bot\College-Information-Chatbot-Information-master\database\questions.txt')
	match = "";
	max_score=0;
